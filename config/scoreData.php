...
// Weekly Query
$sql = $pdo->prepare("SELECT clientid, antivirus, disk, intrusion, usability, services, updates, `event log` as events, `avg score` as overall, date(checkdate) AS date FROM hc_scores_weekly WHERE clientid= :clientid AND checkdate > DATE_ADD(NOW(), INTERVAL -180 DAY);");
